...
def final_fix():
    ...
    try:
        ...
        # Backend
        print("Backend Setup...")
        client.exec_command("cd sweetbomb && python3.10 -m venv venv")
        time.sleep(2)
        # Upgrade pip and install requirements
        stdin, stdout, stderr = client.exec_command("cd sweetbomb && ./venv/bin/python -m pip install --upgrade pip")
        stdout.channel.recv_exit_status()
        stdin, stdout, stderr = client.exec_command("cd sweetbomb && ./venv/bin/python -m pip install -r requirements.txt")
        print(f"Backend requirements status: {stdout.channel.recv_exit_status()}")

        # Frontend
        print("Frontend Setup...")
        stdin, stdout, stderr = client.exec_command("cd sweetbomb/ui/frontend && npm install --registry=https://registry.npmmirror.com && npm run build")
