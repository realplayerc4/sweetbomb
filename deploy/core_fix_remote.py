...
def core_fix_and_install():
    ...
    try:
        ...
        print("Backend Setup...")
        run_sudo_cmd(client, "cd sweetbomb && python3.10 -m venv venv", password)
        run_sudo_cmd(client, "cd sweetbomb && ./venv/bin/python -m pip install --upgrade pip", password)
        run_sudo_cmd(client, "cd sweetbomb && ./venv/bin/python -m pip install -r requirements.txt", password)

        print("Frontend Setup...")
        # Use mirror for speed
        run_sudo_cmd(client, "cd sweetbomb/ui/frontend && npm install --registry=https://registry.npmmirror.com", password)
        run_sudo_cmd(client, "cd sweetbomb/ui/frontend && npm run build", password)
