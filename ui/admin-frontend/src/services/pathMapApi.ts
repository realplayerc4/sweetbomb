...
export async function getPathMap(): Promise<PathMapData> {
  ...
  if (!res.ok) {
    throw new Error(`获取 pathMap 失败: ${res.status}`);
  }
  return res.json();
}
...
export async function getStations(): Promise<{
  pick_stations: Array<{ id: number; connect_node: number }>;
  drop_stations: Array<{ id: number; connect_node: number }>;
}> {
  const res = await fetch(`${API_BASE}/path_map/stations`);
  if (!res.ok) {
    throw new Error(`获取站点列表失败: ${res.status}`);
  }
  return res.json();
}
...
export async function updatePickStation(
  ...
  if (!res.ok) {
    throw new Error(`更新取货站失败: ${res.status}`);
  }
  return res.json();
}
...
export async function updateDropStation(
  ...
  if (!res.ok) {
    throw new Error(`更新放货站失败: ${res.status}`);
  }
  return res.json();
}
