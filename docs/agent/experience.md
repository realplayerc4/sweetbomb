...
**C7**: WebGL混合模式: Three.js 中对于密集放大的色块化点云，使用 `AdditiveBlending` (加色叠加) 会使得多点交接处颜色迅速暴增突破 1.0 (产生纯白高光过曝)，此时应退回使用 `NormalBlending` 以维持本质颜色。

**C8**: IMU 硬件直接解算: RealSense D455 等相机的重力加速度 (`accel`) 向量，可以直接使用 `math.atan2(ay, az)` 和 `math.atan2(-ax, sqrt(y*y+z*z))` 解算出不受漂移影响的静态 Roll 和 Pitch 物理姿态。
...
**D4**: Rsync排除: 同步项目时排除.git/、venv/、**pycache**/、.env避免不必要传输和权限问题。
...
- 从点云发白Bug修复实战学习: C7
- 从真实IMU硬件原生拉流开发中学习: C8
- 实战: 按照 Vibe 工作流重整了项目的里程碑状态与架构内技术记录更新。
