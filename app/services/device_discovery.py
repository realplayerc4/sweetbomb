...
class DeviceDiscovery:
    ...
    def refresh_devices(self) -> List[DeviceInfo]:
        ...
        with self.lock:
            ...
            # 发现新连接的设备
            try:
                # 使用 query_devices() 获取快照，避免迭代时的动态变化异常
                found_devices = self.ctx.query_devices()
                logger.info(f"Checking hardware, query_devices found {len(found_devices)} devices")
                for i in range(len(found_devices)):
                    try:
                        ...
                        logger.info(f"Device found and registered: {device_id} ({device_info.name})")
                    except Exception as dev_e:
                        logger.error(f"Failed to access device at index {i}: {str(dev_e)}")
            except Exception as e:
                # 在无硬件连接或驱动异常时，仅记录日志而不中断启动
                logger.error(f"Hardware discovery critical error: {str(e)}")
