...
async def robot_ws_endpoint(websocket: WebSocket, robot: RobotController = Depends(get_robot_ctrl)):
    """机器人连接状态 WebSocket。
    
    用于实时向前端推送机器人的连通性状态。
    """
    await websocket.accept()
    logger.info(f"机器人状态 WebSocket 已建立: {websocket.client}")
    ...
    try:
        ...
        await websocket.send_json({
            ...
            "connected": status.state != RobotState.ERROR,
            "timestamp": datetime.now().isoformat()
        })
        
        while True:
            # 模拟心跳和状态监听
            # 在真实环境下，这里可以监听 RobotTCPServer 的回调
        ...
        logger.info("机器人状态 WebSocket 已断开")
    except Exception as e:
